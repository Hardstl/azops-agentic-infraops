...
If workload demand profile and/or cost objective is missing, ask before producing any SKU shortlist, recommendation, or option comparison table.
...
5. Confirm cost objective (`lowest-cost`/`balanced`/`performance-first`) before generating options output.
6. If demand is `unknown`, build recommendations for three envelopes (`low`, `medium`, `high`) and mark uncertainty.
7. Search Microsoft Learn docs for each service with `mcp__microsoft_learn__microsoft_docs_search`.
8. Fetch the most relevant pages with `mcp__microsoft_learn__microsoft_docs_fetch`.
9. Build candidate options from documented capabilities only.
10. Include distinct pricing/consumption models where service supports them (for example SQL `DTU` and `vCore`) unless a hard constraint excludes one model.
11. Exclude non-compliant options.
12. Return 2-3 viable options with one clear recommendation.
13. Require explicit user choice.
14. Block authoring until choice is confirmed for every requested service.
...
- Use one blank line between consecutive `param` declarations for readability
...
- Function Apps must have explicit storage backing (newly created or existing) when authored
